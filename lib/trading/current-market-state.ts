...
async function syncTimeWithBinance() {
  try {
    // 获取Binance服务器时间
    const timeResponse: any = await binance.fetchTime();
    const binanceTime: number = Number(timeResponse);
    const localTime = Date.now();
    const timeDifference = binanceTime - localTime;
    
    // 如果时间差超过1秒，则调整本地时间
    if (Math.abs(timeDifference) > 1000) {
      console.log(`时间差: ${timeDifference}ms，正在调整...`);
      // 设置时间偏移量
      binance.options["adjustForTimeDifference"] = true;
      binance.options["timeDifference"] = timeDifference;
    }
  } catch (error) {
    console.warn("无法同步时间:", error);
  }
}
