...
async function testAIWithDefaults() {
  console.log("开始测试AI交易决策输出（带默认值）...");
...
`;
    ...
    const { object, reasoning } = await generateObject({
      ...
      schema: z.object({
        ...
        reasoning: z
          .string()
          .optional()
          .describe("Detailed reasoning for your trading decision"),
      }),
    });
    ...
    console.log("✅ AI交易决策测试（带默认值）成功!");
    
  } catch (error) {
    console.error("❌ AI交易决策测试（带默认值）失败:", error.message);
